...
- **Off:** never runs — not even on `/swarm run`.
...
- The check is created **in progress** as soon as the run starts (so `/swarm run`
...
- With the **GitHub App (OIDC) auth path**, both the comment and the check are posted
  via the Swarm App identity (the same bot that handles `/swarm`), through the Swarm
